...
int Wm100ProtocolHttp::readAnswer(QByteArray &in, uint32_t toRead)
{
    ...
    if (b)
    {
        ...
        io->setOption(2, 0);
        int repeats = 0;
        while (io->writeRead(out, answer, 0, 3000) && res == -1)
        {
            resParse = parseReply(answer, in);
            if (resParse) res = resParse;
            else if (in.size() == toRead) res = 0;
            if (++repeats == 10) res = -35;
        }
    }

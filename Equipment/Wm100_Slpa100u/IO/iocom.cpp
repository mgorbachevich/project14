...
{
    static QString RegexStr("^(serial|usb):\\/\\/([\\w\\-\\/\\.]+)(\\?(\\w+=\\w+)(\\&(\\w+=\\w+))*)?$");
    return RegexStr;
}
...
bool IoCom::writeRead(const QByteArray &out, QByteArray &in, int64_t toRead, int timeoutRead)
{
    bool res = write(out);
    if (res) res = read(in, toRead, timeoutRead);
    return res;
}
